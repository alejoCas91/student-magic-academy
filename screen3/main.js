document.addEventListener("DOMContentLoaded", () => {
  ...
  // for (let i = 0; i < 3; i++) {
  //   prueba.appendChild(createCourseCard());
  // }
  // modalContenpruebt.appendChild(prueba);
  ...
  welcomeP.textContent = 'Sigue aprendiendo!!';


  welcomeDiv.appendChild(welcomeH2);
  welcomeDiv.appendChild(welcomeP);
  for (let i = 0; i < 3; i++) {
    welcomeDiv.appendChild(createCourseCard());
  }
...
document.addEventListener("keyup", e => {
  ...
  if (e.target.matches("#searchInput")) {
    ...
    document.querySelectorAll(".course-card").forEach(Courses => {

      Courses.textContent.toLowerCase().includes(e.target.value.toLowerCase())
        ? Courses.classList.remove("filtro")
        : Courses.classList.add("filtro")
    })
  }
});
